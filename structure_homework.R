...
#ANSWER
rs_joined_orig <- full_join(rs_new,rs_old, join_by(Artist, Song))
nrow(rs_joined_orig)

##issues: 1) we would likely want to rename the columns to illustrate which list they are from,
##instead of the "Year.x" and "Year.y" (similar for rank)
## 2) there are a couple slight character differences in songs that should have been merged
##(ex: "'Stayin' Alive" vs. "Stayin' Alive") 3) I can't tell why others did not merge together
...
#str_squish was listed in the documentation for ?str_trim and seemed like it
#would work better to also eliminate any extra whitespace in between words
...
#ANSWER
rs_new_clean <- rs_all %>% filter(Source=="New")
rs_old_clean <- rs_all %>% filter(Source=="Old")
rs_joined <- full_join(rs_new_clean, rs_old_clean, join_by(Artist,Song), suffix = c("_New", "_Old"))
nrow(rs_joined) #yes, improved!
...
rs_joined <- rs_joined %>%
  ...
  mutate(Rank_Change = Rank_New - Rank_Old) %>%
  arrange(Rank_Change) %>%
  select(Artist, Song, Year_Old, Year_New, Rank_Old, Rank_New, Rank_Change) #also added this so that the rank variables were next to each other
...
#ANSWER
rs_decades <- rs_joined %>%
  mutate(
    Year_New = paste0(floor(Year_New/10)*10, "s"),
    Year_Old = paste0(floor(Year_Old/10)*10, "s"),
    Year_New = as.factor(Year_New),
    Year_Old = as.factor(Year_Old))
#most data have the same year listed, but some are different - going to use Year_New
rs_decades %>%
  group_by(Year_New) %>%
  summarize(avg_change = mean(Rank_Change))
#the 1950s improved the most (average of increasing 118 points)
...
#ANSWER
fct_count(rs_decades$Year_New)
rs_decades$Year_New <- fct_lump_n(rs_decades$Year_New, 3, other_level = "Other")
fct_count(rs_decades$Year_New, prop = TRUE)
...
top20$Release <- parse_date_time(top20$Release, orders = "d-b-Y")
...
#ANSWER
top20 <- left_join(top20, rs_joined, join_by(Artist, Song))

top20 <- top20 %>%
  mutate(
    Month = month(Release, label = TRUE),
    Season = case_when(
...
top20 <- top20 %>%
  mutate(
    Quality = ifelse(str_detect(Key, "m"), "Minor", "Major"),
  )
answer <- top20 %>%
  filter(Quality == "Minor") %>%
  slice_min(Rank_New, n=1)
cat("the top ranked song (#", answer$Rank_New, ") that used a minor key is '", answer$Song, "' by ", answer$Artist)
